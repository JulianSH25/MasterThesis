...
def instance_generator(type, n: int, weighted: bool = False, random_weights: bool = False):
    # Method to CREATE a (random) graph or READ IN and format an existing ADJACENCY LIST
...
def make_line_graph(n: int):
    # Creates line graph for given number of vertices n
    print(f"Generating line graph with {n} nodes...")
    return [(i, i + 1) for i in range(n - 1)]

def make_cycle_graph(n: int):
    # Creates cycle graph for given number of vertices n
...
def make_complete_graph(n: int):
    # Creates complete graph for given number of vertices n
    print(f"Generating complete graph with {n} nodes...")
    return [(i, j) for i in range(n) for j in range(i + 1, n)]

def make_hog_graph(n: int):
    # Read in a 'House of Graphs' instance provided as an adjacency list
