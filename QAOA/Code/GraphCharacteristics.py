...
class GraphCharacteristics:
    ...
    def is_bipartite(edges: list[tuple[int, int]]) -> bool:
        return nx.is_bipartite(GraphCharacteristics._graph(edges))

    @staticmethod
    def is_regular(edges: list[tuple[int, int]]) -> bool:
        graph = GraphCharacteristics._graph(edges)
    ...
    def is_planar(edges: list[tuple[int, int]]) -> bool:
        planar, _ = nx.check_planarity(GraphCharacteristics._graph(edges))
        return planar
