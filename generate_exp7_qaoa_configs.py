...
GENERATE_QAOA_FROM_CACHE = False
...
EXP_NAME = "Exp7_rerun2408"
DEPTH_LIST = [1, 2, 3, 4, 5]
ITERATIONS_LIST = [75]
QAOA_SEED = 42
SDP_SEED = 42
WARM_START_MODE = "amplified"
...
RESULT_NAME_SUFFIX = "_Exp7_AmplifiedWithHeuristic_rerun2408"
...
# Re-run the warm-start families affected by the statevector-ordering fix.
QAOA_FAMILIES = ["L1M1", "L2M2"]
# Generate only the currently missing hybrid caches in this cache-only run.
SDP_CACHE_FAMILIES = ["L2M1"]
...
FAMILY_ORDER = {
    name: i
    for i, name in enumerate(["L1M1", "L2M1", "L2M2", "QAOA_only"])
}
...
def make_warm_config(base: dict, dataset_base: dict, lr: float, use_heuristic: bool,
    ...
    config["warm_start_mode"] = WARM_START_MODE
